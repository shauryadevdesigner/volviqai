...
export const CodeEditor: React.FC<CodeEditorProps> = ({
  ...
  useEffect(() => {
    ...
    const clearAllMarkers = () => {
      monacoRef.current?.editor.getModels().forEach((model: { uri: unknown }) => {
  ...
  const handleEditorMount = (
    ...
    ts?.typescriptDefaults?.addExtraLib(
      `declare module '../ai/design-tokens' {
        export const SPRINGS: {
          luxury: { damping: number; stiffness: number };
          tech: { damping: number; stiffness: number };
          cyber: { damping: number; stiffness: number };
          corporate: { damping: number; stiffness: number };
          editorial: { damping: number; stiffness: number };
          startup: { damping: number; stiffness: number };
        };
        ...
        export const GLOWS: {
          premium: { blur: number; opacity: number; mixBlendMode: 'screen' | 'color-dodge' };
          intense: { blur: number; opacity: number; mixBlendMode: 'screen' | 'color-dodge' };
          ambient: { blur: number; opacity: number; mixBlendMode: 'screen' | 'color-dodge' };
        };
        export const BLURS: {
          sm: string;
          md: string;
    ...
    ts?.typescriptDefaults?.addExtraLib(
      `declare module '../ai/components-library' {
        import React from 'react';
        export const GradientBackground: React.FC<{ bg?: string; glow?: string; accent?: string }>;
        ...
        export const GlassCard: React.FC<{ children: React.ReactNode; style?: React.CSSProperties; delay?: number }>;
    ...
    try {
      ...
      console.warn("Failed to apply initial JSX highlighting:", e);
    }
    ...
    editorInstance.onDidChangeModelContent(() => {
      try {
        ...
        // Suppress highlighting errors when model is switching or null
      }
    });
  };
            ...
            message={
              (() => {
                switch (streamPhase) {
                  ...
                  default: return "Generating code...";
                }
              })()
            }
          />
