...
export function compileCode(code: string): CompilationResult {
  ...
  try {
    ...
    // For 3D code that contains <Canvas>, we need special handling because
    // Remotion hooks (useCurrentFrame, useVideoConfig) don't work inside <Canvas>
    // (R3F uses a separate React store). We detect 3D code and wrap it in a bridge component.
    const is3DModule = (
      repairedCode.includes("<Canvas") ||
      repairedCode.includes("from \"@react-three/fiber\"") ||
      repairedCode.includes("from '@react-three/fiber'")
    ) && (
      repairedCode.includes("mesh") ||
      repairedCode.includes("sphereGeometry") ||
      repairedCode.includes("boxGeometry") ||
      repairedCode.includes("icosahedronGeometry") ||
      repairedCode.includes("torusGeometry") ||
      repairedCode.includes("useFrame") ||
      repairedCode.includes("@react-three/fiber") ||
      repairedCode.includes("@react-three/drei")
    );

    let componentBody: string;
    if (is3DModule) {
      // For 3D code, the AI generates code using Canvas from @react-three/fiber.
      // This doesn't work with Remotion because R3F's Canvas uses a separate React store,
      // which breaks Remotion hooks (useCurrentFrame, useVideoConfig) and causes
      // "R3F: Hooks can only be used within the Canvas component!" errors.
      //
      // The fix: Replace Canvas with ThreeCanvas from @remotion/three.
      // ThreeCanvas is specifically designed to work with Remotion.
      let moduleCode = repairedCode;

      // Replace Canvas import with ThreeCanvas import
      moduleCode = moduleCode.replace(
        /import\s*\{\s*Canvas\s*\}\s*from\s*["']@react-three\/fiber["'];?/g,
        'import { ThreeCanvas } from "@remotion/three";'
      );
      // Also handle: import { Canvas as MyCanvas } from "@react-three/fiber"
      moduleCode = moduleCode.replace(
        /import\s*\{\s*Canvas\s+as\s+\w+\s*\}\s*from\s*["']@react-three\/fiber["'];?/g,
        'import { ThreeCanvas } from "@remotion/three";'
      );
      // Also handle: import { Something, Canvas } from "@react-three/fiber"
      moduleCode = moduleCode.replace(
        /import\s*\{([^}]*),\s*Canvas\s*\}\s*from\s*["']@react-three\/fiber["'];?/g,
        'import { $1 } from "@react-three/fiber"; import { ThreeCanvas } from "@remotion/three";'
      );
      // Also handle: import { Canvas, Something } from "@react-three/fiber"
      moduleCode = moduleCode.replace(
        /import\s*\{\s*Canvas,\s*([^}]*)\}\s*from\s*["']@react-three\/fiber["'];?/g,
        'import { $1 } from "@react-three/fiber"; import { ThreeCanvas } from "@remotion/three";'
      );

      // Strip remaining @react-three/fiber imports (Canvas is the only thing we need to replace)
      moduleCode = moduleCode.replace(/import\s*\{[^}]*\}\s*from\s*["']@react-three\/fiber["'];?/g, "");

      // Strip all other imports
      moduleCode = moduleCode.replace(/import\s+type\s*\{[\s\S]*?\}\s*from\s*["'][^"']+["'];?/g, "");
      moduleCode = moduleCode.replace(/import\s+\w+\s*,\s*\{[\s\S]*?\}\s*from\s*["'][^"']+["'];?/g, "");
      moduleCode = moduleCode.replace(/import\s*\{[\s\S]*?\}\s*from\s*["'][^"']+["'];?/g, "");
      moduleCode = moduleCode.replace(/import\s+\*\s+as\s+\w+\s*from\s*["'][^"']+["'];?/g, "");
      moduleCode = moduleCode.replace(/import\s+\w+\s*from\s*["'][^"']+["'];?/g, "");
      moduleCode = moduleCode.replace(/import\s*["'][^"']+["'];?/g, "");
      moduleCode = moduleCode.trim();

      // Remove export keywords
      moduleCode = moduleCode.replace(/export\s+default\s+(function|class)\b/g, "$1");
      moduleCode = moduleCode.replace(/export\s+(const|let|var|function|class)\b/g, "$1");
      moduleCode = moduleCode.replace(/export\s+default\s+(?!function\b|class\b)\w+\s*;?/g, "");
      moduleCode = moduleCode.replace(/export\s*\{\s*[\s\S]*?\}\s*;?/g, "");

      // Replace <Canvas with <ThreeCanvas and </Canvas> with </ThreeCanvas>
      moduleCode = moduleCode.replace(/<Canvas\b/g, '<ThreeCanvas');
      moduleCode = moduleCode.replace(/<\/Canvas>/g, '</ThreeCanvas>');

      // Find all component names defined in the code
      const componentNames: string[] = [];
      const componentRegex = /const\s+([A-Z][a-zA-Z0-9_]*)\s*=\s*\(/g;
      let match;
      while ((match = componentRegex.exec(moduleCode)) !== null) {
        componentNames.push(match[1]);
      }

      // Return the last defined component (usually the main one)
      const returnExpr = componentNames.length > 0
        ? `return ${componentNames[componentNames.length - 1]};`
        : 'return null;';
      componentBody = `${moduleCode}\n${returnExpr}`;
    } else {
      ...
      componentBody = validatedBody;
      if (repairs.length > 0) {
        console.log(`[Compiler] JSX auto-repairs applied: ${repairs.join("; ")}`);
      }
    }

    // Detect R3F hooks called at the top level (before return statement)
    // This catches AI-generated code that uses useFrame/useThree outside Canvas,
    // even if the code also contains <Canvas> in the returned JSX
    const returnMatch = componentBody.search(/return\s*[<(]/);
    const topLevelPart = returnMatch >= 0 ? componentBody.substring(0, returnMatch) : componentBody;
    const needsCanvasWrap = !is3DModule && (/\buseFrame\s*\(/.test(topLevelPart) || /\buseThree\s*\(/.test(topLevelPart));

    let wrappedSource: string;
    if (is3DModule) {
      // For 3D modules, the component body contains the full module code.
      // We need to evaluate it and find the exported component.
      // Find all component names defined in the code
      const componentNames = [];
      const componentRegex = /const\s+([A-Z][a-zA-Z0-9_]*)\s*=\s*\(/g;
      let match;
      while ((match = componentRegex.exec(componentBody)) !== null) {
        componentNames.push(match[1]);
      }
      // Return the last defined component (usually the main one)
      const returnExpr = componentNames.length > 0
        ? `return ${componentNames[componentNames.length - 1]};`
        : `return typeof VolviqAnimation !== 'undefined' ? VolviqAnimation : (typeof DynamicAnimation !== 'undefined' ? DynamicAnimation : (typeof MyAnimation !== 'undefined' ? MyAnimation : null));`;
      wrappedSource = `${componentBody}\n${returnExpr}`;
    } else {
      wrappedSource = needsCanvasWrap
        ? `const __R3FComponent = () => {\n${componentBody}\n};\nconst DynamicAnimation = () => (<Canvas><__R3FComponent/></Canvas>);`
    ...
    // Safe useFrame wrapper: catches "hooks outside Canvas" errors
    const safeUseFrame = (callback: any, renderPriority?: any): any => {
      try {
    ...
    // Safe useThree wrapper: returns defaults when outside Canvas context
    const safeUseThree = (selector?: any): any => {
      try {
        return useThree(selector);
      } catch {
        const defaults = {
          camera: { position: { x: 0, y: 0, z: 8 }, fov: 50 },
          gl: {} as any,
          scene: {} as any,
          ...
          invalidate: () => {},
          advance: () => {},
          clock: { elapsedTime: 0, getElapsedTime: () => 0, getDelta: () => 0 },
          get: () => defaults,
          raycaster: {} as any,
          pointer: { x: 0, y: 0 },
          performance: { current: 1, min: 1, max: 1, regress: () => {} },
          internal: { active: false, priority: 0, frames: 0 },
          xr: {} as any,
          events: {} as any,
          clear: () => {},
          fov: 50,
          ...
          toJSON: () => ({}),
          clone: () => defaults.camera,
          copy: () => defaults.camera,
          add: () => {},
          remove: () => {},
          removeFromParent: () => {},
          clearParents: () => {},
        ...
        return selector ? selector(defaults) : defaults;
      }
    };
    ...
    const SafeReact = {
      ...
      createElement: function (type: any, props: any, ...children: any[]) {
        // Only modify style for HTML elements (string types like 'div', 'span')
        // R3F elements (components, forwardRef, etc.) must pass through unmodified
        // to preserve React context propagation for useFrame/useThree hooks
    ...
    // For 3D code, use original React to preserve R3F context propagation
    // SafeReact wrapper breaks R3F's Canvas context for useFrame/useThree hooks
    const is3D = (code.includes("Canvas") || code.includes("useFrame") || code.includes("useThree")) && (
      code.includes("mesh") ||
