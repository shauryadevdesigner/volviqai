...
import {
  getGenerationModeDirective,
  normalizeGenerationMode,
  resolveGenerationFormat,
...
interface GenerateRequest {
  ...
  frameImages?: string[];
  /** User-uploaded multimodal assets */
...
export async function POST(req: Request) {
  ...
  // Fuse prompt with uploaded visual asset context if any
  const prompt = uploadedAssets.length > 0 
    ? fusePromptWithAssets(rawPrompt, uploadedAssets)
    : rawPrompt;

  // Combine frame images and uploaded image URLs for vision context
  const allImages = [
    ...frameImages,
    ...uploadedAssets.filter(a => a.type === "image").map(a => a.url)
  ];

  const resolvedMode = normalizeGenerationMode(generationMode);
  const resolvedFormat = resolveGenerationFormat({
    mode: resolvedMode,
    isFollowUp,
    fps,
    durationInFrames,
  });
  const modeDirective = getGenerationModeDirective(
    resolvedMode,
    resolvedFormat.fps,
    resolvedFormat.durationInFrames,
  );
  ...
  const enhancedSystemPrompt = `${SYSTEM_PROMPT}\n\n## AUTHORITATIVE GENERATION FORMAT\n${modeDirective}${
...
You are an elite motion graphics engineer. The user wants to modify the existing component.
      ...
      async start(controller) {
        ...
        try {
          const finalCode = await runOrchestrator({
            ...
            durationInFrames: resolvedFormat.durationInFrames,
            onEvent: (event) => {
