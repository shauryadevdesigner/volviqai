...
import { useScrollReveal } from '../hooks/useCinematicScroll';
import { usePerformanceProfile } from '../hooks/usePerformanceProfile';

const Generator = () => {
  const { lowEffects } = usePerformanceProfile();

  // GSAP stagger reveal for list items
    ...
    <section className="relative py-32 px-margin-mobile md:px-margin-desktop border-b border-outline-variant bg-surface" id="capabilities">
      ...
      <div className="max-w-container-max mx-auto">
        <div className="flex flex-col md:flex-row gap-16 md:gap-24 items-center">
          ...
          {/* Left panel: 3D perspective entrance — slides from left with rotateY */}
          <motion.div
            className="w-full md:w-1/2 relative perspective-child"
            initial={{ opacity: 0, x: -120, scale: 0.9, rotateY: 8 }}
            ...
            >
              ...
              >
                <div className="p-4 relative w-full h-full">
                  ...
                  {/* Ken Burns slow drift on image */}
                  <div className="overflow-hidden">
                    <img 
                      alt="Holographic data streams and nodes" 
                      className="w-full h-auto img-mono transition-all duration-700 group-hover:filter-none cinema-ken-burns" 
                      src="/cyberpunk.jpg"
                    />
                  </div>
                  
                  {/* Animated label — wipe from left */}
                  <motion.div
                    className="absolute top-8 left-8 font-code-md text-code-md text-primary bg-black/60 px-3 py-1.5 backdrop-blur-sm border border-outline-variant"
                    initial={{ clipPath: 'inset(0 100% 0 0)' }}
                    whileInView={{ clipPath: 'inset(0 0% 0 0)' }}
                    viewport={{ once: true }}
                    transition={{ duration: 0.8, ease: ease.inOutCubic, delay: 0.6 }}
                  >
                    SYS.GEN.01
                  </motion.div>
          ...
          {/* Right panel: slides from right with delay */}
          <motion.div
            className="w-full md:w-1/2 space-y-8 text-left"
            initial={{ opacity: 0, x: 80, scale: 0.95 }}
            whileInView={{ opacity: 1, x: 0, scale: 1 }}
            viewport={{ once: true, margin: '-80px' }}
            transition={{ duration: 1.0, ease: ease.outExpo, delay: 0.2 }}
          >
            <motion.div
              className="font-label-sm text-label-sm text-on-surface-variant tracking-widest uppercase flex items-center gap-2"
              initial={{ opacity: 0, x: 20 }}
              whileInView={{ opacity: 1, x: 0 }}
              viewport={{ once: true }}
              transition={{ duration: 0.5, ease: ease.outExpo, delay: 0.3 }}
            >
              <span className="w-1.5 h-1.5 bg-primary rounded-full"></span>
              02 // SYNTHESIS
            </motion.div>
            
            {/* BIG headline — camera push-in from scale 1.2 */}
            <motion.h2
              className="font-headline-xl text-headline-xl text-primary uppercase leading-tight tracking-tight"
            ...
            {/* small subtitle — after headline settles */}
            <motion.p
            ...
            {/* List items — GSAP spring overshoot stagger */}
            <ul ref={listRef} className="space-y-6 pt-8 border-t border-surface-variant">
              {listItems.map((item, idx) => (
                ...
                >
                  <span className="material-symbols-outlined text-primary p-2 bg-surface-container-low border border-outline-variant group-hover:border-primary transition-colors duration-500 ease-out-expo">
                    {item.icon}
                  </span>
                  <div>
                    <h4 className="font-label-md text-label-md text-on-surface tracking-wider uppercase group-hover:text-primary transition-colors duration-500 ease-out-expo">
                      {item.title}
                    </h4>
