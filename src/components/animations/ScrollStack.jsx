...
const ScrollStack = ({
  ...
  const cacheOffsets = useCallback(() => {
    ...
    cardOffsetsRef.current = cardsRef.current.map(card => {
      if (!card) return 0;
      if (useWindowScroll) {
        return getAbsoluteOffsetTop(card);
      } else {
    ...
    if (endElement) {
      if (useWindowScroll) {
        endElementOffsetRef.current = getAbsoluteOffsetTop(endElement);
      } else {
  ...
  useLayoutEffect(() => {
    ...
    // Cache offsets after a brief timeout and requestAnimationFrame to let fonts/layouts settle
