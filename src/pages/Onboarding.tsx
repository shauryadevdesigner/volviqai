...
import {
  completeOnboarding,
  getSupabase,
  getMotionDashboardRedirectWithSession,
} from '../lib/supabase';
import {
  BUSINESS_TYPES,
  PLATFORMS,
...
export default function Onboarding() {
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      window.location.href = getMotionDashboardRedirectWithSession(session);
    } catch (err) {
        ...
        <section className="flex min-h-screen flex-col items-center justify-center px-margin-mobile py-24 md:px-margin-desktop">
          ...
          <GlareHover className="w-full max-w-lg rounded-2xl border border-outline-variant bg-surface-container-low/90 p-8 backdrop-blur-md md:p-10">
            ...
            <div className="mt-8 flex gap-3">
              ...
              >
                {loading ? 'Saving…' : step === TOTAL_STEPS ? 'Complete Setup' : 'Continue'}
              </button>
            </div>
          </GlareHover>
        </section>
